...
assert.equal(so.questionCount, 5652);
assert.deepEqual(so.chapterGroups.map(group => group.deckIds.length), [12, 20, 16, 10, 9, 9, 12]);
...
try {
  ...
  assert.deepEqual(await page.locator("#deck-filter .deck-filter-count").allTextContents(), ["1,002問", "1,265問", "1,288問", "883問", "417問", "357問", "440問"]);
  ...
  assert.equal(await picker(7).locator("input:checked").count(),12);
  await page.reload(); await shown("setup-panel"); await settled();
  assert.equal(await picker(7).locator("input:checked").count(),12);
  await page.locator("#question-limit").fill("1");
  await page.locator("#start-study").click(); await shown("study-shell");
  ...
  assert.equal(await page.locator('.chapter-picker input:checked').count(),88);
  await page.locator("#start-study").click(); await shown("study-shell");
  assert.equal(await page.locator("#subject-name").textContent(),"世界史SO｜7デッキ");
  assert.ok([...sessions.values()].some(session=>session.deckIds?.length===88));
  assert.deepEqual(errors,[]);
  assert.equal(requests.some(url=>/\/v1\/.*(speech|rating-sound)/.test(url)),false);
  console.log("複数章の画面確認完了：7デッキ・88パート・5,652問、章とパートの選択、68問の時期問題、保存復元、本文・回答・評価、既存章、全パートの学習、狭い画面、音声停止");
} finally {
