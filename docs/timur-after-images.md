...
v0.184（2026年9月5日）作成、v0.187で隣接コマの写り込み除去と各登場人物の中心軸格子合わせを実施。内蔵の画像生成を使用した。外部APIを直接使う方法は使用していない。

## 保存先

すべて `public/images/timur-after/` に保存。人物は128×192画素、建物・道具・動物は192×192画素の透過PNG。
生成した画像集を升目ごとに切り出し、最近傍方式で縮小した。v0.187にて、升目の境界からはみ出していた隣接コマの破片（浮島）を除去し、人物の中心軸を128画素の中央（64画素）へ揃える格子調整を行って、アニメーション時や表情変化時の位置飛びを解消した。生成された透明度を維持しており、ブラウザーから画像生成サービスへ接続することはない。
