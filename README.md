...
現在は、「大学受験・最重要」の世界史100用語・333問を使って学習できます。

1つの用語について質問と答えを順番に確認し、すべて終わると統合説明を表示します。質問数は固定せず、元データに存在する分だけ表示します。

画面上部の「シャッフル」から、用語を登録順で学ぶ通常モードと、出題順を入れ替えるシャッフルモードを切り替えられます。質問の順番はどちらのモードでも変わりません。

## フォルダ構成

- `public`：Web上で公開するページと見た目の設定
- `public/data`：CSVから生成したWeb表示用の分割データ
- `data/source`：科目ごとの元CSV（各科目で現在使用するCSVを1ファイルだけ配置）
...
`data/source/world-history`内の元CSVを更新したあと、次の命令でWeb表示用データを作り直します。ファイル名が変わっても、同じ場所にCSVが1ファイルだけあれば自動的に認識します。
