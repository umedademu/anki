...
const startBlock = app.match(
  /async function start\(\)[\s\S]*?\n}\n\ndocument\.getElementById\("edit-questions"\)/,
)?.[0];
...
if (!html.includes('<script src="/app.js?v=0.278" type="module"></script>')) {
...
if (
  ...
  !html.includes("v0.278") ||
  !app.includes("app-navigation.js?v=0.278") ||
  !changelog.includes("v0.278") ||
  !settingsHtml.includes("v0.278") ||
  !historyHtml.includes("v0.278") ||
  !analysisHtml.includes("v0.278")
) {
  throw new Error("開始前の条件選択画面、更新情報ページ、版番号が揃っていません。");
}
if (
  !html.includes('id="mindset-player-panel"') ||
  !html.includes('id="mindset-toggle"') ||
  !html.includes('id="mindset-speech-rate"') ||
  !html.includes('id="mindset-interval-seconds"') ||
  !html.includes('id="mindset-study-time"') ||
  !html.includes('id="mindset-completion-panel"') ||
  !html.includes('id="mindset-completion-time"') ||
  ...
  !app.includes("function tickMindsetStudyClock(") ||
  !app.includes("function startNewMindsetStudyScreen(") ||
  !app.includes("state.mindsetScreenStudySeconds < state.studyTimeLimitSeconds") ||
  !app.includes("state.mindsetStudySeconds") ||
...
if (
  ...
  !settingsHtml.includes('id="routine-video-form"') ||
  !settingsHtml.includes('id="routine-video-list"') ||
  ...
  !app.includes("applyStudyRoutineVideoSkip") ||
  !app.includes("saveRoutineMultiplier") ||
  !app.includes("saveRoutineVideoSkip") ||
  !app.includes("routineMultiplier: multiplier") ||
  !app.includes("routineSkipVideos: skipVideos") ||
  ...
  !worker.includes('Object.hasOwn(patch, "routineSkipVideos")') ||
...
if (
  ...
  !beginStudyBlock?.includes("buildQueue();") ||
  beginStudyBlock?.includes("savedSession.tasks") ||
...
if (
  ...
  !app.includes("function setDeckOptions(decks, selectedIds)") ||
  !app.includes("function selectedDeckIds()") ||
  !app.includes("async function activateDecks(deckIds, { keepDeckSelection = false } = {})") ||
  ...
  !app.includes("async function activateSubject(subjectId, requestedDecks = [])") ||
...
if (
  !html.includes('href="/styles.css?v=0.278"') ||
...
if (
  ...
  !startBlock.includes("readAppRoute(window.location.search)") ||
  !startBlock.includes("navigateToRoute(initialRoute") ||
...
if (
  ...
  !html.includes('id="study-menu-review-shared"') ||
  !html.includes('id="study-menu-review-custom"') ||
...
if (
  ...
  !html.includes('value="listen-answer"') ||
...
if (
  ...
  !renderQuestionBlock.includes('`読み：${term.reading}`') ||
  !renderQuestionBlock.includes('`意味：${answerDisplayText}`') ||
  !renderQuestionBlock.includes(
    "state.answerVisible && (Boolean(explanation) || showsYearMnemonic)",
  ) ||
  !styles.includes(".question-reading") ||
  !styles.includes("font-size: clamp(1.05rem, 2vw, 1.18rem)") ||
  !styles.includes("font-weight: 850") ||
  !styles.includes(
